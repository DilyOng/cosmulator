...
def mmd(target_samples, emulated_samples, weights=None, *,
        max_points=1000, n_permutations=100, seed=0):
    ...
    the isotropic kernel sees comparable scales per parameter. A permutation null
    (shuffle the P/q labels, recompute) gives a p-value -- but only when the target
    weights are uniform, since permutation needs the pooled points to be
    exchangeable under the null. For a *weighted* target (the usual nested-sampling
    case) the statistic is still returned but ``pvalue`` is ``None``; a valid
    weighted null (multiplier/wild bootstrap) is a planned addition.
    ...
    max_points : int
        ...
        the posterior peak.
    n_permutations : int
        Number of label permutations for the null. ``0`` skips the p-value.
    seed : int
    ...
    # The permutation null needs the pooled points to be EXCHANGEABLE under H0.
    # That holds when both samples are i.i.d. draws from the same distribution --
    # i.e. when the target weights are uniform. When the target is a *weighted*
    # nested-sampling measure and the emulator is unweighted i.i.d. draws, the two
    # groups are not exchangeable, so label-permutation does not give a calibrated
    # p-value. In that case we report the statistic but no p-value; a valid
    # weighted null (e.g. a multiplier/wild bootstrap) is the next increment.
...
def certify(target_samples, emulated_samples, weights=None, *,
    ...
    guaranteed to shrink as sample size grows even for a good emulator, and the
    warn only fires when a valid p-value exists (uniform-weight targets); for a
    weighted target MMD is reported but does not drive the verdict yet.
    """
